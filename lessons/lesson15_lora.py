...
─────────────────
  ...
  【为什么低秩有效？直觉理解】
    ...
    想象你有一个4096维的空间，模型微调需要在这个空间里"调整方向"。

    全参数微调：可以在4096个方向上任意调整
    → 但实际上，微调任务通常只需要调整少数几个方向
    → 比如从"通用对话"微调到"医疗问答"，只需要调整"医疗术语"相关的方向

    类比：
      全参数微调 = 装修时可以改所有墙、地板、天花板（4096个方向）
      LoRA = 装修时只刷墙、换窗帘（8个方向就够了）
      → 因为大部分结构不需要改，只需要调整少数"风格"

    数学直觉：
      预训练模型已经学到了通用的语言知识（语法、逻辑、常识）
      微调只需要在这些知识上做"小幅调整"
      小幅调整 = 低秩变化（只有少数方向需要改变）

    实验证据：
      研究发现，微调时 ΔW 的"有效秩"通常只有 8-64
      即 4096 维空间中，只有 8-64 个方向真正在变化
      其余 4000+ 个方向几乎不变
      → 用 rank=8 的 LoRA 就能捕获 99% 的有效变化
