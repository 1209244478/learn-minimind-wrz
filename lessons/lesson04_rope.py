...
# 【为什么要把 d 维分成 d/2 组？逐步理解】
#
# 2D 旋转（已学）：
#   一个 2D 向量 [x0, x1] 做一次旋转
#   → 完美编码了1个位置信息
#
# 4D 旋转（关键过渡）：
#   一个 4D 向量 [x0, x1, x2, x3]
#   → 分成2组：[x0,x1] 和 [x2,x3]
#   → 第1组用频率 θ0 旋转（捕捉细粒度位置）
#   → 第2组用频率 θ1 旋转（捕捉粗粒度位置）
#   → 两组旋转叠加，同时编码了"近处"和"远处"的位置关系
#
# d 维旋转（一般化）：
#   一个 d 维向量分成 d/2 组
#   → 每组2个维度，做一次 2D 旋转
#   → d/2 个不同频率，从高频到低频
#   → 高频组区分相邻位置（位置1 vs 位置2）
#   → 低频组区分远处位置（位置1 vs 位置1000）
#
# 类比：
#   想象一个时钟：
#   秒针（高频）= 区分1秒内的变化
...
#   时针（低频）= 区分1小时内的变化
#   三根针一起看，就能精确定位时间
#   RoPE 的 d/2 组频率就像 d/2 根"指针"，精确定位位置
