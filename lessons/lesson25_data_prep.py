...
print("-" * 50)
print("""
1. 预训练数据（纯文本，模型学习"下一个词是什么"）：
   ─────────────────────────────────────────
   {"text": "自然语言处理是人工智能领域中的一个重要方向，它研究如何让计算机理解和生成人类语言。自然语言处理的应用包括机器翻译、文本摘要、问答系统等。近年来，基于Transformer的大语言模型在自然语言处理领域取得了突破性进展。"}
   ─────────────────────────────────────────
   特点：纯文本，没有问答格式，模型学习语言的统计规律

2. SFT 数据（指令-回答格式，模型学习"听指令"）：
   ─────────────────────────────────────────
   {"messages": [
     {"role": "system", "content": "你是一个有帮助的AI助手。"},
     {"role": "user", "content": "请解释什么是梯度下降"},
     {"role": "assistant", "content": "梯度下降是一种优化算法，用于最小化损失函数。它沿着损失函数的负梯度方向更新参数，每次更新一步，逐步接近最优解。公式为：θ = θ - α·∇L(θ)，其中α是学习率，∇L是梯度。"}
   ]}
   ─────────────────────────────────────────
   特点：有明确的"指令"和"回答"，模型学习按指令行事

3. DPO 数据（偏好对，模型学习"什么是好回答"）：
   ─────────────────────────────────────────
   {"prompt": "如何学习编程？",
    "chosen": "建议从Python开始，它语法简洁，社区资源丰富。可以从官方教程入手，边学边练，逐步掌握基础概念后再尝试小项目。",
    "rejected": "编程很难，不建议学。"}
   ─────────────────────────────────────────
   特点：同一个问题有好坏两个回答，模型学习偏好好的那个
""")
