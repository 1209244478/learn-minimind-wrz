...
【DPO vs RLHF：为什么要用 DPO？】

  想象你是一个老师，要教学生写好作文：

  RLHF 的做法（复杂）：
    1. 先训练一个"评分老师"（奖励模型），学会给作文打分
    2. 学生写作文 → 评分老师打分 → 学生根据分数调整
    3. 问题：需要4个模型（学生、标准答案、评分老师、评分标准）
    4. 训练不稳定，学生可能学会"骗分"而不是真正写好

  DPO 的做法（简单）：
    1. 直接给学生看两篇作文："这篇好，那篇差"
    2. 学生自己调整，让自己更像好的那篇
    3. 只需要2个模型（学生 + 标准答案）
    4. 训练稳定，不会"骗分"

  为什么 DPO 能替代 RLHF？
    数学上，DPO 和 RLHF 优化的是同一个目标函数！
    RLHF：max E[r(x,y)]  （最大化奖励）
    DPO：max E[log σ(β * (log_ratio_chosen - log_ratio_rejected))]  （直接优化偏好）

    只是 DPO 把"奖励函数"用 log 概率比的形式隐式表达了，
    不需要显式训练奖励模型。
"""
...
【逐步推导：从 RLHF 到 DPO】
  ...
  第3步：反解奖励函数
    ...
    因为 log Z 与 y 无关，在偏好比较中会消掉，所以：
    r(x,y) = β * log(π(y|x) / π_ref(y|x))  （省略常数项）

  第4步：代入 Bradley-Terry 模型
  ...
  这就是 DPO 损失！整个推导的核心洞察：
    "不需要显式的奖励模型，log 概率比就是隐式的奖励"
