...
7. DeepSeek-R1 (2025.1):
   - 推理时计算（Inference-Time Compute）的里程碑
   - 通过 RL 训练让模型学会"思考"（生成思维链）
   - 证明小模型+推理时间 > 大模型直接生成
   - 开源推理模型浪潮的起点

8. GPT-5 (2025):
   - 原生多模态融合（文本+图像+音频+视频统一预训练）
   - 百万 Token 上下文窗口
   - 内置推理引擎，自适应推理深度

9. Claude 4 / Gemini 2.5 (2025):
   - Claude 4: MCP 协议（Model Context Protocol），Agent 标准化
   - Gemini 2.5: 推理时计算标杆，Test-Time Compute 架构

--- 2026 年（最新前沿）---

10. DeepSeek-V4 (2026.4):
    - 1.6T 总参数 / 49B 激活（MoE 极致稀疏）
    - V4-Flash: 284B 总参数 / 13B 激活（极致性价比）
    - 原生 100 万 Token 上下文窗口
    - DSA2 稀疏注意力：推理算力仅为 V3.2 的 10%，KV Cache 仅 7%
    - mHC（流形约束超连接）：替代残差连接，改善梯度流
    - FP4 专家权重：原生支持 Blackwell 硬件
    - 混合注意力：滑动窗口 + 压缩机制（4:1 top-k / 128:1 dense）

11. GPT-5.4 / 5.5 / 5.6 (2026):
    - GPT-5.4: 原生 Computer Use（模型直接操作电脑）
    - 150 万 Token 上下文窗口（GPT-5.6）
    - 幻觉率降低 52.5%，事实准确性提升 33%
    - 内置 Codex 性能，无需独立编码模型

12. Claude Opus 4.6 / Mythos 5 (2026):
    - 自适应推理框架：effort 参数动态调整推理深度
    - 服务端上下文压缩：自动摘要旧对话，实现"无限对话"
    - 128K 输出 Token：单次生成完整技术文档
    - Cybench 安全测试 100%（代码漏洞检测超人）

13. Gemini 3.1 Pro (2026):
    - Nodal Reasoning（节点推理）：并行探索多个冲突假设
    - 生成"推理森林"而非线性思维链
    - GPQA Diamond 94.3%（博士级科学推理）
    - ARC-AGI-2 77.1%（抽象推理，逼近人类 85-90%）

14. 2026 三大范式变革:
    a) 推理时计算成为标配:
       - 从"训练时堆算力"到"推理时多思考"
       - o3/R1/DeepSeek-V4 均支持推理深度可调
       - IDC 预测: 2026 年底超 60% 企业级 AI 采用此架构
    b) Agentic AI 元年:
       - Agent 从"会聊天"到"会干活"
       - MCP/A2A 协议标准化 = Agent 时代的 TCP/IP
       - 多智能体协作（编码+审计+文档并行执行）
       - 具身智能：搭载大模型的通用服务机器人商用
    c) 开源模型逼近闭源:
       - DeepSeek-V4 开源 1.6T 模型
       - Qwen3.6 / GLM-5.1 / Llama 4 开源万亿级
       - 中国开源模型 Hugging Face 下载量增长 300%+

15. 2026 架构新趋势:
    - 计算-记忆解耦: Engram（静态知识存廉价 DRAM，释放 HBM）
    - 液态 MoE: DeepSeek-V4 的 LMoE，推理时动态调整激活参数
    - 线性复杂度架构: Mamba/SSM/RWKV 成为长文本标配
    - 原生多模态: 文本/图像/音频/视频统一编码器（非拼接）
    - 小模型爆发: 3B/7B/13B 边缘部署，性能逼近 60B 稠密模型
