...
# 【新增】路由崩塌问题演示
print("\n" + "-" * 50)
print("【路由崩塌问题：为什么需要 aux_loss？】")
print("-" * 50)
print("""
MoE 的一个严重问题：路由崩塌 (Router Collapse)

  什么是路由崩塌？
    门控网络可能只选择1-2个专家，其余专家永远不会被用到。
    这就像一个公司有8个员工，但每次任务都只找同1-2个人做，
    其他人永远闲着——资源浪费！

  为什么会发生？
    1. 初始时，某个专家碰巧被选中，得到了训练
    2. 训练后的专家更强 → 下次更可能被选中
    3. 正反馈循环 → 只有少数专家被反复选中
    4. 其他专家永远得不到训练 → "富者越富"

  没有 aux_loss 时的路由分布（示意）：
    专家0: ████████████████████ 80% 的token
    专家1: ████ 15% 的token
    专家2: █ 3% 的token
    专家3: ▏ 2% 的token
  ...
  aux_loss 的数学原理：
    load = 每个专家被选中的频率
    gate_scores.mean(0) = 每个专家的平均门控分数
    aux_loss = Σ (load_i × gate_score_i) × num_experts × coef

    当负载不均衡时（某个专家load高且score高），aux_loss变大
    → 梯度会"惩罚"不均衡，鼓励门控网络均匀分配

  router_aux_loss_coef 的选择：
    太小（0.001）：约束不够，仍然崩塌
    太大（0.1）：过度约束，专家无法专业化
    常用值：0.01（MiniMind 默认值）
""")
