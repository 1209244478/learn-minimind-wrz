...
═══════════════════════════════════════════
  ...
  实验1：把 n_layers 从 4 改成 2，观察 loss 变化
    → 层数减少，模型容量变小，loss 可能更高

  实验2：把 dim 从 512 改成 256，观察生成质量
    → 维度减半，参数量约减4倍，生成可能更差

  实验3：把训练轮数从 1000 改成 100，观察效果
    → 训练不足，模型还没学好就停了

  实验4：把学习率从 3e-4 改成 1e-2，观察训练过程
    → 学习率太大，loss 可能震荡甚至 NaN

  实验5：把 temperature 从 0.8 改成 0.1 和 2.0，对比生成结果
    → 0.1: 几乎确定性输出（重复、保守）
    → 2.0: 非常随机（可能乱码）

  实验6：把 n_kv_heads 从 2 改成 8（=n_heads），对比 KV Cache 大小
    → GQA → MHA，KV Cache 变大但效果可能更好

  每个实验只需要改一个数字，重新运行就能看到效果！
  这就是学大模型最好的方式：动手试，看结果。
""")
